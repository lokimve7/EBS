...
| 01-6 | [Tasks/01-6.md](../Tasks/01-6.md) | OpenAI API 공식문서를 기준으로 `스크립트 생성` 버튼 클릭 시 입력한 주제, 분위기, 스타일, 장면 수를 바탕으로 장면별 스크립트 데이터를 받아오고, 응답 개수만큼 씬 카드를 생성해 화면에 표시한다. | 예정 |
...
- 2026-06-30: `node --check server.js`, `node --check public/scripts/main.js` 정적 문법 점검을 완료했고, `Tasks/01-5.md`, `docs/ARCHITECTURE.md`, `docs/PENDING_ISSUES.md`를 실제 구현 기준으로 갱신했다.
- 2026-06-30: 사용자 확인 중 모달이 처음부터 노출되는 문제를 수정하기 위해 `public/styles/main.css`에 `hidden` 상태 우선 규칙을 추가했다.
- 2026-06-30: 사용자 확인 중 API 키 설정 모달의 하단 버튼이 화면 밖으로 밀리는 문제를 줄이기 위해 모달 폭과 내부 간격, 카드 높이, 버튼 크기를 압축 조정했다.
- 2026-06-30: 사용자가 API 키 설정 모달 표시와 저장/취소 결과를 확인해 `01-5`를 완료 처리했다.
- 2026-06-30: `01-6` 후보가 `01-5` 다음 단계로 적합한지 검토했고, OpenAI API 기반 스크립트 생성 task 초안을 작업리스트에 추가했다.
